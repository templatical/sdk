...
The resolver is called once per committed src value (typing in the src input is debounced, so partial values never reach it) and results are cached per src for the editor instance's lifetime, including failures. Merge-tag srcs are never passed to the resolver; their `placeholderUrl` preview is resolved instead.
