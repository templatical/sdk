...
Match exported rich-text spacing to the editor canvas
...
The renderer now emits one scoped rule for the elements a rich-text block
stores, via `<mj-style inline="inline">` — so the declarations arrive as real
inline styles that hold up in clients which strip `<style>` from the head. The
scope (`tpl-rich-text`, on paragraph and title blocks only) deliberately leaves
an `html` block's own markup untouched.

Both sides now read one set of values, `RICH_TEXT_SPACING` in
`@templatical/types`, and a parity test fails if the canvas CSS drifts from it.
