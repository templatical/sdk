...
| `resolvePreview`    | `ResolvePreview`                                                  | No       | Resolves the template for preview surfaces — typically evaluating logic tags against real data on your backend. Display-only: never in `getContent()`, a send or an export. Supersedes `MergeTag.sample`. See [Merge Tags](/guide/merge-tags#resolving-previews-with-real-data) |
