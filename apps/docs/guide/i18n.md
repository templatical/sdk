...
1. Copy `packages/vue/src/i18n/locales/en.ts` to a new file (e.g., `fr.ts`)
2. Translate all string values, keeping the same key structure
3. Register the locale in `packages/vue/src/i18n/index.ts`
...
```ts
// packages/vue/src/i18n/locales/fr.ts
export default {
  blocks: {
    text: 'Texte',
    image: 'Image',
