...
Most people want exactly one of the two — that's why they're separate skills. Authoring or editing a template is `templatical-email`; wiring the editor into your own codebase is `templatical-sdk`. "Build a welcome email and wire it into my app" wants both, in that order: `templatical-email` builds and validates the template JSON first, then `templatical-sdk` mounts the editor and loads it in. Each skill's `SKILL.md` names the other, so an agent with both installed hands off between them on its own.

There is no backend and no API key for either: your agent is the inference. Nothing is sent to Templatical.

## Install

Both skills are plain [Agent Skills](https://agentskills.io) folders, so any agent that reads `SKILL.md` can use them.

```bash
npx skills add templatical/sdk
```

One command installs **both** skills. [`skills`](https://github.com/vercel-labs/skills) detects which supported agents you have and installs each one straight into the directory that agent reads, so there is nothing to place by hand.
...
If your agent doesn't pick a skill up afterwards, check that it is enabled in the agent's own skill list.
