...
Already have a template in another editor? The skill can convert **Unlayer**, **BeeFree**, **HTML**, **MJML**, and **Topol** emails into Templatical JSON — point it at the file and it writes a working template plus a short report of what converted cleanly and what fell back to raw HTML (import is lossy by nature). Then preview it live and refine the rough edges into native blocks. To run the converters directly in your own code instead, see the migration guides: [Unlayer](/guide/migration-from-unlayer), [BeeFree](/guide/migration-from-beefree), [HTML](/guide/migration-from-html), [MJML](/guide/migration-from-mjml), [Topol](/guide/migration-from-topol).
