...
| `linkUnderline` | `boolean` | Whether links are underlined document-wide. Defaults to `false`. |
...
Links follow the same cascade: set a document `linkColor` to apply one color to every link (rich-text and menu), or leave it unset so links inherit the surrounding text color. `linkUnderline` toggles underlines document-wide. A per-block or per-item color (e.g. a Menu item's color) still overrides the document link color.
