...
| `templateNameField` | `boolean`                                                         | Nein     | Zeigt den Vorlagennamen im Header, direkt bearbeitbar. Standardwert `true`. Auf `false` setzen, wenn Ihr Store keine Namensspalte hat oder Ihre eigene Bedienoberfläche den Namen verwaltet — `create({ name })`, `setName()` und der `name` in jedem Speicher-Patch funktionieren weiter |
