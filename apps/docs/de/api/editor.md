...
| `resolvePreview`    | `ResolvePreview`                                                  | No       | Löst die Vorlage für Vorschauflächen auf — typischerweise durch Auswerten von Logik-Tags mit echten Daten in Ihrem Backend. Nur zur Anzeige: nie in `getContent()`, im Versand oder im Export. Hat Vorrang vor `MergeTag.sample`. Siehe [Vorschau-Rendering](/de/guide/preview-rendering) |
