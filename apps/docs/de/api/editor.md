...
| `layout`             | `TemplateContent`                                                 | Nein     | Hülle des Hosts, angewendet in Vorschau und `toMjml()` / `toHtml()`. Nie in `getContent()`, Speichern, Verlauf oder die Bearbeitungsleinwand geschrieben. Ein `TemplateContent` mit genau einem `slot`. Wirft, wenn der Slot in einem Wrapper sitzt und die Vorlage `mj-wrapper` erzeugt. Siehe [Layout](/de/guide/layout)                                                                                                                                                                                                   |
| `sectionWrapper`     | `boolean`                                                         | Nein     | Ob die Sektions-Symbolleiste Wrapper hinzufügen anbietet. Weggelassen / `true` ist an. `false` blendet das Steuerelement aus, auch dort, wo ein Wrapper zulässig ist. Bereits eingeschaltete Wrapper bleiben erreichbar. Nur Darstellung: Ausblenden entfernt `section.wrapper` nie. Unabhängig von `layout`. Siehe [Layout](/de/guide/layout)                                                                                                                                                                               |
...
| `onRequestMedia`     | `(context?: MediaRequestContext) => Promise<MediaResult \| null>` | Nein     | UI-Überschreibung für die Medienauswahl (Bynder, Cloudinary-Widget, ein Host-Modal). Hat Vorrang vor `media`, wenn beide gesetzt sind. Gibt `{ url, alt? }` oder `null` zurück. Siehe [Bilder](/de/guide/images) und [Medien](/de/backend/media)                                                                                                                                                                                                                                                                                                                                           |
...
| `mergeTags`          | `MergeTagsConfig`                                                 | Nein     | Merge-Tag-Konfiguration. Jedes Tag kann ein optionales `sample` tragen — einen Beispielwert, den Vorschauen an seiner Stelle anzeigen. `showRawValue: false` hält den rohen Token aus der Autoren-Oberfläche heraus. Siehe [Merge-Tags](/de/guide/merge-tags)                                                                                                                                                                                                                                                                                                                              |
...
| `media`              | `MediaProvider`                                                   | Nein     | Speicher-Backend für die Medienbibliothek — die Auswahl hinter Durchsuchen an Bildfeldern, Video-Thumbnails und Bildfeldern benutzerdefinierter Blöcke. Der Editor stellt Modal, Raster, Zuschnitt und Einfügen; Sie stellen die Persistenz. `list` ist erforderlich; jede andere Methode ist `false \| fn`. Dasselbe Objekt trägt auch `onCreated` / `onUpdated` / `onDeleted`. Weglassen lässt Bildfelder URL-only, sofern `onRequestMedia` nicht gesetzt ist. `createLocalStorageMediaProvider()` bietet eine Variante ohne Backend. Siehe [Medien](/de/backend/media)                  |
...
| `socialIconsBaseUrl` | `string`                                                          | Nein     | Basis-URL für die Social-Icon-PNGs im exportierten MJML, weitergereicht an [`socialIconsBaseUrl`](/de/api/renderer-typescript#social-icons) des Renderers. Setzen Sie sie, um die Icons selbst zu hosten; ohne Angabe gilt der versionsgebundene jsDelivr-Standard. Betrifft nur den lokalen `toMjml()` / `toHtml()`-Pfad (über `compileMjml`) — eine `render.toMjml`-Implementierung übergibt die URL an ihr eigenes `renderToMjml`                                                                                                                                                       |
...
| `locale`             | `string`                                                          | Nein     | Locale-Code (z. B. `'en'`, `'de'`, `'pt-BR'`, `'es'`, `'ca'`, `'fr'`, `'nl'`, `'ja'`). Standardwert ist `'en'`                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
::: tip Cloud-Editor
Der Cloud-Editor stellt **beide** Methoden bereit, aufgelöst über Clouds serverseitigen Renderer (oder Ihren eigenen `render`-Provider). Beachten Sie: Cloud rendert das _gespeicherte_ Template, jeder Aufruf speichert also zuerst.
:::
