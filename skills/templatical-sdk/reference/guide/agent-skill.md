...
The skill is a folder containing `SKILL.md` — an [open standard](https://agentskills.io) that Claude Code, Codex CLI, Cursor, Gemini CLI and others all read.
...
[`skills`](https://github.com/vercel-labs/skills) detects which supported agent(s) you have and installs the skill straight into the right directory for each — one command instead of a per-agent copy. It reports anonymous usage telemetry by default (which repo and skill you installed, for public repositories); set `DISABLE_TELEMETRY=1` or `DO_NOT_TRACK=1` first if you'd rather it didn't.
