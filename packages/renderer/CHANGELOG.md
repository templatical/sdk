...
- 99de7a2: Fix: the unrenderable-block marker could emit a stray `--` inside its HTML comment.

  `renderUnrenderableBlock` wraps a block's `type` and `id` in an HTML comment, and `commentSafe` scrubs both first. It collapsed hyphen runs _before_ stripping `<` / `>`:
...
- 956664e: Escape table cell content when rendering, and strip HTML from imported BeeFree table cells.
