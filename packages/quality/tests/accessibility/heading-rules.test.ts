...
describe("a11y.heading-empty", () => {
  it("fires when content has no text", () => {
    const block = createTitleBlock({ content: "<p></p>", level: 2 });
    const issues = lint([block]).filter((i) => i.ruleId === "a11y.heading-empty");
  ...
  it("fires when content is whitespace tags", () => {
    const block = createTitleBlock({ content: "<p>   </p>", level: 2 });
    expect(lint([block]).filter((i) => i.ruleId === "a11y.heading-empty")).toHaveLength(1);
  });

  it("does not fire when content has text", () => {
    const block = createTitleBlock({ content: "<p>Hello</p>", level: 2 });
    expect(lint([block]).filter((i) => i.ruleId === "a11y.heading-empty")).toEqual([]);
  });
});

describe("a11y.heading-skip-level", () => {
  it("fires on H1 → H3 jump", () => {
    ...
    const issues = lint([h1, h3]).filter(
      (i) => i.ruleId === "a11y.heading-skip-level",
    );
    expect(issues).toHaveLength(1);
  ...
  it("does not fire on H1 → H2", () => {
    ...
    expect(
      lint([h1, h2]).filter((i) => i.ruleId === "a11y.heading-skip-level"),
    ).toEqual([]);
  });
  ...
  it("does not fire when stepping down (H3 → H2)", () => {
    ...
    expect(
      lint([h2, h3, h2b]).filter((i) => i.ruleId === "a11y.heading-skip-level"),
    ).toEqual([]);
  });
  ...
  it("looks through nested sections in document order", () => {
    ...
    expect(
      lint([h1, section]).filter((i) => i.ruleId === "a11y.heading-skip-level"),
    ).toHaveLength(1);
  });
});

describe("a11y.heading-multiple-h1", () => {
  it("fires when more than one H1 exists", () => {
    ...
    const issues = lint([h1a, h1b]).filter(
      (i) => i.ruleId === "a11y.heading-multiple-h1",
    );
    expect(issues).toHaveLength(1);
  ...
  it("does not fire on single H1", () => {
    const h1 = createTitleBlock({ content: "<p>A</p>", level: 1 });
    expect(
      lint([h1]).filter((i) => i.ruleId === "a11y.heading-multiple-h1"),
    ).toEqual([]);
  });
});
