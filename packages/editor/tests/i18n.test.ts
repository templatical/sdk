...
import { useCloudI18n, useCloudI18nStrict } from '../src/composables/useCloudI18n';
...
describe('OSS locale parity', () => {
  ...
  const enEntries = getNestedEntries(en).sort(([a], [b]) =>
    a.localeCompare(b),
  );
...
describe('useI18n', () => {
  ...
  describe('format', () => {
    ...
    it('formats actual translation strings', () => {
      expect(
        format(cloudEn.header.templatesUsed, { used: 3, max: 10 }),
