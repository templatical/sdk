...
export interface MergeTagPopupOpenOptions {
  ...
  items: MergeTag[];
  /**
   * Anchor rect getter (the caret rect). Viewport coordinates, re-read on
   * every reposition so the popup tracks the caret through scroll. Same
   * contract the TipTap suggestion plugin uses for `clientRect`.
   */
 ...
 * repositioning, and cleanup.
 *
 * TipTap-free on purpose. Field autocomplete (`useMergeTagAutocomplete`)
 * lives on the editor's eager graph — the properties panel mounts with the
 * chrome — and a value import of `@tiptap/core` / `@tiptap/suggestion` from
 * this module would pull the whole rich-text stack into every session.
 * The TipTap extension in `extensions/MergeTagSuggestion.ts` is the adapter
 * that maps ProseMirror `SuggestionProps` onto this controller, and it loads
 * only when a title or paragraph enters edit mode.
 */
