  ...
  return {
    ...
    closeBundle() {
      try {
        ...
        // If dist layout shifts or the entry walk fails, warn and let the
        // build continue rather than failing CI on a reporting plugin.
        const message = err instanceof Error ? err.message : String(err)
        this.warn?.(`bundle-stats: skipped (${message})`)
      }
    },
  }
}
