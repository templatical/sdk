...
- 5758c24: Background fills can be transparent
...
- a2b4cde: Merge tags are re-picked rather than hand-edited
...
- a41a573: Add `editor.setMergeTags(tags)` for replacing merge tags after `init()`
...
- 3704f11: Merge-tag fields expose their controls as a group, not a button inside a button
...
- a2cad4a: Show a minted merge tag's label instead of its raw token
...
- 39c7c1e: Match exported rich-text spacing to the editor canvas, and make the paragraph gap settable per block
  ...
  The renderer emits one rule per _distinct_ gap in the document and the canvas
  reads the same number from a CSS variable on the block wrapper, so the two
  cannot disagree.

  Scoped to the space _between_ paragraphs inside one block. Space around a block
...
- e2790cc: Image blocks take a corner radius
...
- Updated dependencies [e2790cc]
  - @templatical/renderer@0.30.0
  - @templatical/media-library@0.30.0
  - @templatical/quality@0.30.0

## 0.29.1

### Patch Changes

- 6b0b3bc: Fix two ways the host page could break the editor
...
- @templatical/media-library@0.29.1
  - @templatical/quality@0.29.1
  - @templatical/renderer@0.29.1
...
- bfa416d: Fix input fields overflowing their container in the template settings panel and other sidebars (issue #115).
  ...
  Root cause: Tailwind preflight is disabled (intentional), so `box-sizing: border-box` was never applied to form elements. The hand-rolled `.tpl` reset block reset `font-family` and button chrome but not `box-sizing`. With `tpl:w-full` (`width: 100%`) plus a horizontal padding utility like `tpl:px-3.5` (28px total), inputs resolved to `content-box` and extended their padding beyond the parent — most visible in the 320px right sidebar.
