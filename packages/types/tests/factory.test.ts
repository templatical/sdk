...
describe("block factory functions", () => {
  ...
  it("applies a wrapper (outer frame) override on a section block", () => {
    ...
    expect(block.wrapper?.padding).toEqual({
      top: 10,
      right: 10,
      bottom: 10,
      left: 10,
...
describe("createBlock", () => {
  ...
  it("throws for unknown block type", () => {
    expect(() => createBlock("unknown" as never)).toThrow(
      "Unknown block type: unknown",
    );
  });

  it("throws for custom block type (requires definition)", () => {
    expect(() => createBlock("custom" as never)).toThrow(
      "Unknown block type: custom",
    );
  });
});
  ...
  it("recursively clones deeply nested sections (section within section)", () => {
    ...
    if (cloned.type === "section") {
      ...
      if (clonedInner.type === "section") {
        ...
        expect(clonedInnerChild.id).toMatch(
          /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/,
        );
      }
    }
...
describe("createCustomBlock with defaultStyles", () => {
  ...
  it("still populates fieldValues alongside defaultStyles", () => {
    const definition: CustomBlockDefinition = {
      type: "styled-fields",
      name: "Styled Fields",
      fields: [{ key: "title", label: "Title", type: "text", default: "Hi" }],
      template: "<div>{{title}}</div>",
      defaultStyles: {
...
describe("createBlock with blockDefaults", () => {
  ...
  it("replaces arrays in table rows", () => {
    const customRows = [{ id: "r1", cells: [{ id: "c1", content: "Custom" }] }];
    const defaults: BlockDefaults = {
  ...
  it("applies menu defaults", () => {
    ...
    if (block.type === "menu") {
      ...
      // No baked-in color — a new menu inherits the document textColor.
  ...
  it("returns default block when blockDefaults is undefined", () => {
    ...
    if (block.type === "title") {
      expect(block.level).toBe(2);
      // No baked-in color — a new title inherits the document textColor.
      expect(block.color).toBeUndefined();
    }
  });
