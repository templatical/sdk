...
import { computed, reactive, readonly, type DeepReadonly, type Ref } from '@vue/reactivity';
...
export function useEditor(options: UseEditorOptions): UseEditorReturn {
    const state = reactive<EditorState>({
        content: options.content ?? createDefaultTemplateContent(options.defaultFontFamily),
    ...
    function addBlock(
        ...
        if (targetSectionId) {
            const section = findBlockById(
                state.content.blocks,
                targetSectionId,
            );
            if (section && section.type === 'section') {
                section.children[columnIndex] =
                    section.children[columnIndex] || [];
