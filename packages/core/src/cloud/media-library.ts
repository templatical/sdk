...
import { MediaApiClient } from './media-api';
...
export function useMediaLibrary(options: UseMediaLibraryOptions) {
        ...
        try {
            ...
            if (currentFolderId.value === null) {
                ...
                items.value = items.value.map(
                    (item) => movedMap.get(item.id) ?? item,
                );
            } else {
    ...
    async function replaceFile(file: File): Promise<MediaItem | null> {
        ...
        try {
            const updated = await api.replaceMedia(
                pendingReplaceItem.value.id,
                file,
            );
    ...
    return {
        items, folders, currentFolderId, viewMode, searchQuery, categoryFilter,
        sortOption, isLoading, isUploading, uploadProgress, hasMore, selectedItems,
        previewItem, frequentlyUsedItems, deleteUsageInfo, showDeleteWarning,
        loadItems, loadMore, search, filterByCategory, sortBy, navigateToFolder,
        showFrequentlyUsed, uploadFile, uploadFiles, moveSelected, updateFile,
        deleteSelected, isImportingFromUrl, importFromUrlError, importFromUrl,
        toggleSelection, clearSelection, selectItem, loadFolders, createFolder,
        renameFolder, deleteFolder, findFolderInTree, loadFrequentlyUsed,
        checkUsageBeforeDelete, confirmDelete, cancelDelete, isReplacing,
        replaceError, showReplaceWarning, pendingReplaceItem, replaceUsageInfo,
        checkUsageBeforeReplace, cancelReplace, replaceFile, replaceMediaDirectly,
    };
}
