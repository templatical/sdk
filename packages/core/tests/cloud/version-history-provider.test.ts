 ...
 *
 * These cases pin the `content` hint, which is what keeps scrubbing
 * synchronous for a Cloud session and is easy to drop by accident when
  ...
  describe("list", () => {
    it("maps the wire shape onto the contract, hint included", async () => {
      vi.mocked(ApiClient.prototype.getVersions).mockResolvedValue([
        record({ id: "ver-2", isAutomatic: false }),
        record({ id: "ver-1" }),
      ]);
    ...
    it("keeps the endpoint order rather than sorting", async () => {
      vi.mocked(ApiClient.prototype.getVersions).mockResolvedValue([
        record({ id: "a", createdAt: "2026-01-01T00:00:00Z" }),
        record({ id: "b", createdAt: "2026-09-09T00:00:00Z" }),
      ]);
  ...
  describe("create", () => {
    it("forwards the label and maps the result back", async () => {
      vi.mocked(ApiClient.prototype.createVersion).mockResolvedValue(
        record({ id: "ver-9", isAutomatic: false }),
      );

      const created = await (
        setup().create as Exclude<ReturnType<typeof setup>["create"], false>
  ...
  describe("restore", () => {
    it("returns the template the endpoint produced", async () => {
      ...
      const template = await (
        setup().restore as Exclude<ReturnType<typeof setup>["restore"], false>
      )("tmpl-1", "ver-1");
