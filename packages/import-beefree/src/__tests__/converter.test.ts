...
describe('convertBeeFreeTemplate', () => {
    it('throws for invalid input', () => {
        expect(() =>
            convertBeeFreeTemplate({} as BeeFreeTemplate),
        ).toThrow('Invalid BeeFree template');
    ...
    it('flattens 4+ columns with warning', () => {
        ...
        expect(
            content.blocks.some((b) => b.type === 'spacer'),
        ).toBe(true);
        expect(report.warnings.some((w) => w.includes('4 columns'))).toBe(
            true,
        );
    });
    ...
    it('converts real BeeFree fixture', () => {
        const { content, report } =
            convertBeeFreeTemplate(fixture as BeeFreeTemplate);
    ...
    it('warns about multiple web fonts', () => {
        ...
        const { report } = convertBeeFreeTemplate(template);
        expect(report.warnings.some((w) => w.includes('web fonts'))).toBe(
            true,
        );
    });
});
