...
const MediaLibraryModal = defineAsyncComponent(
    () => import('./components/MediaLibraryModal.vue'),
);
...
const themeVarMapping: Record<keyof ThemeOverrides, string> = {
    bg: '--tpl-bg', bgElevated: '--tpl-bg-elevated', bgHover: '--tpl-bg-hover', bgActive: '--tpl-bg-active',
    border: '--tpl-border', borderLight: '--tpl-border-light',
    text: '--tpl-text', textMuted: '--tpl-text-muted', textDim: '--tpl-text-dim',
    primary: '--tpl-primary', primaryHover: '--tpl-primary-hover', primaryLight: '--tpl-primary-light',
    secondary: '--tpl-secondary', secondaryHover: '--tpl-secondary-hover', secondaryLight: '--tpl-secondary-light',
    success: '--tpl-success', successLight: '--tpl-success-light',
    warning: '--tpl-warning', warningLight: '--tpl-warning-light',
    danger: '--tpl-danger', dangerLight: '--tpl-danger-light',
...
provide('allowCustomConditions', props.config.displayConditions?.allowCustom ?? false);
...
provide('projectId', computed(() => authManager.projectId));
...
const snapshotHistoryInstance = shallowRef<UseSnapshotHistoryReturn | null>(null);
...
const snapshotHistorySnapshots = computed(() => snapshotHistoryInstance.value?.snapshots.value ?? []);
const snapshotHistoryIsLoading = computed(() => snapshotHistoryInstance.value?.isLoading.value ?? false);
const snapshotHistoryIsRestoring = computed(() => snapshotHistoryInstance.value?.isRestoring.value ?? false);
...
async function handleSnapshotNavigate(snapshot: TemplateSnapshot): Promise<void> {
...
async function confirmRestoreSnapshot(): Promise<void> {
    ...
    await snapshotHistoryInstance.value.restoreSnapshot(previewingSnapshot.value.id);
...
const canSendTestEmail = computed(() => planConfigInstance.hasFeature('test_email'));
const hasTemplateSaved = computed(() => !!editor.state.template?.id);
const isWhiteLabeled = computed(() => planConfigInstance.hasFeature('white_label'));
const templateLimit = computed(() => planConfigInstance.config.value?.limits.max_templates ?? null);
const templateCount = computed(() => planConfigInstance.config.value?.template_count ?? 0);
...
async function initialize(): Promise<void> {
    ...
    try {
        ...
        // Update fonts
        props.fontsManager.setCustomFontsEnabled(planConfigInstance.hasFeature('custom_fonts'));
        ...
        // Apply theme
        if (props.config.theme && planConfigInstance.hasFeature('theme_customization')) {
...
function getErrorMessage(error: Error): string {
    if ('isUnauthorized' in error && (error as { isUnauthorized: boolean }).isUnauthorized) {
...
function isNotFoundError(error: Error): boolean {
    return 'isNotFound' in error && !!(error as { isNotFound: boolean }).isNotFound;
}
...
async function preRenderCustomBlocks(content: TemplateContent): Promise<void> {
    const renderBlock = async (block: Block): Promise<void> => {
        if (isCustomBlock(block)) {
            ...
            try {
                customBlock.renderedHtml = await registry.renderCustomBlock(customBlock);
            } catch {
    ...
    >
        ...
        >
            ...
            >
                ...
                >
                    <div class="tpl-shimmer tpl:h-5 tpl:w-28 tpl:rounded-[var(--tpl-radius-sm)]"></div>
                    <div class="tpl:flex tpl:gap-3">
                        <div class="tpl-shimmer tpl:h-8 tpl:w-20 tpl:rounded-[var(--tpl-radius-sm)]"></div>
                        <div class="tpl-shimmer tpl:h-8 tpl:w-20 tpl:rounded-[var(--tpl-radius-sm)]"></div>
                    </div>
                </div>
                <!-- Skeleton body -->
                <div class="tpl:flex tpl:flex-1 tpl:overflow-hidden">
                    ...
                    >
                        <div v-for="n in 5" :key="n" class="tpl-shimmer tpl:size-7 tpl:rounded-[var(--tpl-radius-sm)]"></div>
                    </div>
                    <!-- Canvas area -->
                    <div
                    ...
                    >
                        ...
                        >
                            ...
                            <div class="tpl:py-4">
                                <div class="tpl-shimmer tpl:h-44 tpl:w-full tpl:rounded-[var(--tpl-radius-sm)]"></div>
                            </div>
                            <div class="tpl:space-y-2 tpl:py-4">
                                <div class="tpl-shimmer tpl:h-3 tpl:w-full tpl:rounded"></div>
                            ...
                            <div class="tpl:flex tpl:justify-center tpl:py-4">
                                <div class="tpl-shimmer tpl:h-10 tpl:w-36 tpl:rounded-[var(--tpl-radius-sm)]"></div>
                            </div>
                            <div class="tpl:space-y-2 tpl:py-4">
                                <div class="tpl-shimmer tpl:mx-auto tpl:h-2.5 tpl:w-1/2 tpl:rounded"></div>
                                <div class="tpl-shimmer tpl:mx-auto tpl:h-2.5 tpl:w-1/3 tpl:rounded"></div>
                            </div>
                        </div>
                    </div>
                    <!-- Right panel -->
                    ...
                    >
                        <div class="tpl-shimmer tpl:h-8 tpl:rounded-[var(--tpl-radius-sm)]"></div>
        ...
        >
            ...
            >
                ...
                >
                    <AlertCircle :size="32" :stroke-width="1.5" style="color: var(--tpl-danger)" />
                </div>
                <div class="tpl:flex tpl:flex-col tpl:items-center tpl:gap-2 tpl:text-center">
        ...
        >
            ...
            <div class="tpl-header-left tpl:flex tpl:min-w-[200px] tpl:items-center tpl:gap-3">
                <div
                    v-if="!isWhiteLabeled"
                ...
                >
                    {{ format(t.header.templatesUsed, { used: templateCount, max: templateLimit }) }}
                </span>
            </div>

            <!-- Center: viewport + preview + dark mode + collaboration + snapshots -->
            <div class="tpl-header-center tpl:flex tpl:items-center tpl:justify-center tpl:gap-10">
                <ViewportToggle
            ...
            <!-- Right: Cloud actions -->
            <div class="tpl-header-right tpl:flex tpl:min-w-[200px] tpl:items-center tpl:justify-end tpl:gap-3">
                <div
                ...
                <button
                    ...
                    :style="{
                        backgroundColor: commentsOpen ? 'var(--tpl-primary)' : 'transparent',
                ...
                <button
                    class="tpl-btn tpl:inline-flex tpl:items-center tpl:gap-1.5 tpl:rounded-[var(--tpl-radius-sm)] tpl:border tpl:px-3.5 tpl:py-2 tpl:text-sm tpl:font-medium tpl:whitespace-nowrap tpl:transition-all tpl:duration-[120ms] tpl:ease-[cubic-bezier(0.16,1,0.3,1)] hover:tpl:bg-[var(--tpl-primary)] hover:tpl:text-white tpl:disabled:cursor-not-allowed tpl:disabled:opacity-50"
                    style="background-color: transparent; color: var(--tpl-primary); border-color: var(--tpl-primary)"
                    :disabled="editor.state.isSaving || isSaveExporting || !editor.state.isDirty"
                    @click="saveTemplate()"
                >
                    <Save v-if="!editor.state.isSaving && !isSaveExporting" :size="16" :stroke-width="2" />
                    <Loader2 v-else class="tpl-spinner" :size="16" :stroke-width="2" />
                    {{ editor.state.isSaving || isSaveExporting ? t.header.saving : t.header.save }}
                </button>
            </div>
        </header>
        ...
        >
            <div class="tpl:flex tpl:items-center tpl:gap-2 tpl:text-sm" style="color: var(--tpl-text)">
        ...
        <RightSidebar
            ...
            @update-block="(updates) => editor.updateBlock(editor.selectedBlock.value!.id, updates)"
            @delete-block="blockActions.deleteBlock(editor.selectedBlock.value!.id)"
            @duplicate-block="blockActions.duplicateBlock(editor.selectedBlock.value!)"
        ...
        <AiChatSidebar
            :visible="aiChatOpen"
            :on-apply="(content: TemplateContent) => {
                history.record();
        ...
        <SaveModuleDialog
            v-if="planConfigInstance.hasFeature('saved_modules') && props.config.modules !== false"
