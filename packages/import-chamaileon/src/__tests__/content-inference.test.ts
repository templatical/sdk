...
describe("inferTextBlock", () => {
  ...
  it("infers a sole h1 as a title", () => {
    expect(
      inferTextBlock(
        '<h1 style="text-align: center;">WELCOME TO MY COMPANY</h1>',
      ),
    ).toEqual({
      kind: "title",
      level: 1,
      inner: "WELCOME TO MY COMPANY",
    });
  });

  it("infers a paragraph otherwise", () => {
    const r = inferTextBlock('<p style="text-align: center;">Hello</p>');
...
describe("stripTags", () => {
  ...
  it("decodes the entities Chamaileon emits", () => {
  ...
  it("strips a 4.x button label wrapped in a paragraph", () => {
    expect(stripTags("<p>Details</p>\n")).toBe("Details");
  });
});
